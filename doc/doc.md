...
| `$this` | the value being processed when processing embedded templates |
...
| `slug` | Transform a string to a "slug" that is safe to be used as an IRI path segment by replacing reserved characters with an underscore(_) and replacing consecutive runs of white-space with a single hyphen (-). |
...
| `smap_to('template')` | Process an object or list of objects in the same way as `map_to` but without any inherited properties from the outer context. The processing of the template will not have access to any properties defined on the parent object or any of the global context properties. |
| `to_entries` | Expand an object into a list of key/value mappings using the properties `$key` and `$value`. |
| `reconcile(...)` | Reconcile the value, see later. |
| `autoCV` or `autoCV(name,type)` | Replace a string value with a generated concept whose prefLabel is the string value. Will emit the auto-generated concept scheme. If no `name` is given then it uses the name of the property being generated as the name of the concept scheme. The optional `type` argument can be either `hash` to use a hash of the label as the local part of the concept URI or `label` to use the normalized label directly (the default). |
...
The `mappings` can be included in imports, to make it easy to create reusable mappings.
